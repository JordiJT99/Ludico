...
Daily edition assembly now includes quiz, verdadero/falso and crossword. Verdadero/falso uses the same server-verified question-attempt pipeline with an explicit three-to-twenty-question, two-option validation rule, so its private truth values never reach the client before close. Adivina la palabra and sopa de letras remain generated, validated and visible as reserve/admin content, but are not published until their own attempt contracts are ready.
