...
| US-017 | en progreso | ledger inmutable `xp-v1` por finalización, bonus idempotente por segundo tipo diario, nivel y dos logros cosméticos derivados, migración PostgreSQL, contrato/API privado y panel web; falta mostrar la progresión en móvil y repetir E2E de cuenta |
...
iOS publicado → archivo avanzado/estadísticas → social privado → rewarded ads casuales → premium/restore → experimentos. Cada bloque se reespecifica con señales MVP. Fase 3: más juegos, torneos/patrocinios, países/idiomas, recomendación, invitados y widgets.
