...
| US-002 | completada localmente | `pnpm check`, workflow CI configurado con SCA/secret scan (pendiente de primera ejecución remota), hooks, `.env.example`, builds web/Expo/API/worker |
| US-003 | en progreso | migración/seed y restore aislado verificados en PGlite y PostgreSQL 18 local; CI PostgreSQL 18 configurado pero pendiente de ejecución remota, pg-boss/retry/DLQ/singleton; falta restore real PostgreSQL 18 en staging |
| US-010 | en progreso | DST, publish/close idempotente, reserva manual, kill switch, outbox/audit, solution gate, API/web/mobile y edición diaria contra PostgreSQL 18 local; falta E2E completo con PostgreSQL 18/staging |
| US-011 | en progreso | token 256-bit/hash, rotación con sujeto estable por linaje, expiración, revocación, cookie HttpOnly, SecureStore y XOR de GameAttempt verificado; falta E2E en dispositivo |
| US-012 | en progreso | contrato 5–15×4, score servidor antifraude, progreso deduplicado/versionado, submit idempotente, resultado sin soluciones, UI web/native y E2E web con teclado, offline y reconexión; faltan E2E en dispositivo y auditoría a11y completa |
...
| US-020 | en progreso | jobs 21 días idempotentes, presupuesto/coste, adapter intercambiable, schema/invariantes, fuentes HTTPS por ítem, bloqueos, duplicado exacto y similitud tokenizada acotada, cuarentena de riesgo, puertos de verificación/evaluación, alerta estructurada de reserva <10 y circuit breaker por proveedor/tipo con contadores; `fake` genera fixtures explícitos y `disabled` falla cerrado; faltan proveedor/DPA real, verificador externo y proveedor alternativo probado |
