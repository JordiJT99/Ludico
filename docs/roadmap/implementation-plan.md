...
| US-020 | en progreso | reserva dinámica de 14 días por tipo más candidato en curso, emergencia determinista validada, presupuesto/coste, adapter intercambiable, schema/invariantes, fuentes HTTPS por ítem, bloqueos, duplicado exacto y similitud tokenizada acotada, cuarentena de riesgo, puertos de verificación/evaluación, alertas estructuradas y circuit breaker por proveedor/tipo con contadores; `fake` genera fixtures explícitos y `disabled` evita IA externa sin interrumpir la reserva; faltan proveedor/DPA real, verificador externo y proveedor alternativo probado |
