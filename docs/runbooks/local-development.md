...
El seed es repetible y publica los cinco retos sintéticos para el día actual en `Europe/Madrid`. Para fijar otra edición, definir `SEED_DATE=AAAA-MM-DD` antes de ejecutarlo.

### Pipeline de contenido y backoffice

`AI_PROVIDER=disabled` es el valor seguro para proveedores externos: el worker conserva la edición diaria mediante el generador determinista con coste cero y la API de planificación humana responde no disponible. `AI_PROVIDER=fake` habilita fixtures sintéticos explícitos para desarrollo. `AI_JOB_BUDGET_MICROS` limita cada job y un exceso lo marca fallido. Cualquier proveedor distinto de `disabled|deterministic|fake` impide arrancar hasta que exista un adapter revisado.
...
El worker mantiene catorce días aprobados por tipo más un candidato en curso, dentro de un horizonte de 21 días; genera por cola con retry/DLQ y ensambla la edición siguiente con los cinco juegos aprobados. Sin el conjunto completo, la transacción no crea una edición parcial; si falta una clase de contenido, se reencola una variante determinista de emergencia, la valida y reintenta el ensamblado. El pipeline separa payload público/solución, conserva lineage, fuentes, hash, findings, coste, auditoría y outbox. Una fuente sintácticamente HTTPS no basta para autoaprobar: los puertos de verificación y evaluación deben devolver éxito; en otro caso el candidato queda `pending_review`.

El superadmin puede modificar en el backoffice la hora local de apertura/cierre, la hora de planificación y el objetivo de reserva (7–21 días). El worker lee esos ajustes en cada minuto de planificación y en cada ensamblado; tras la hora configurada, la primera instancia que llega reclama el plan diario de forma idempotente, incluso después de un reinicio. Las ediciones ya creadas conservan su ventana original para no alterar una competición en curso.
